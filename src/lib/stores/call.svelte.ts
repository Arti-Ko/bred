...
/**
 * Раздача своего плеера в комнату.
 *
 * Спрятана целиком: кнопка, которая включает трансляцию звука из вашего
 * плеера. Слушать чужой плеер приложение по-прежнему умеет — убрана только
 * возможность начать раздачу самому. Вернуть — поставить здесь `true`.
 */
export const SHARE_PLAYER: boolean = false;
