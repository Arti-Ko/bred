...
<aside class="pane left">
  ...
  <div class="pane-body">
    ...
    {:else if session.spaceId}
      <!--
        Пространство есть, а каналов нет — значит история ещё не приехала.
        Каналы не создаются при входе: они приходят от того, кто уже внутри.
        Раньше здесь была немая пустота, и человек считал, что всё сломалось.
      -->
      <p class="hint">
        история ещё не приехала.<br />
        каналы придут от того, кто вас позвал — <b>он должен быть в сети</b><br />
        {#if session.neighbors === 0}
          сейчас на связи никого{:else}на связи: {session.neighbors}{/if}
      </p>
    {/if}
