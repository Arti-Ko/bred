...
///
/// Это страховка от единственной потерянной рассылки, а не основной путь
/// доставки: обычное сообщение приезжает через рой за доли секунды. Пять секунд
/// — это верхняя граница задержки в худшем случае, и она же предел того, как
/// долго человек смотрит на пустое место, не понимая, почему собеседник
/// «печатает» в никуда.
...
impl Net {
    ...
    ///
    /// Событие уходит в рой ровно один раз. Присутствие и «печатает» ходят по
    /// кругу каждые десять секунд, поэтому потерянный удар сердца сам собой
    /// заменяется следующим, а потерянное сообщение не заменяется ничем. Отсюда
    /// и жалоба, которая звучит невозможно: «в сети» видно, «печатает» видно, а
    /// сообщений нет.
    ///
    /// Второй шанс у события был только один — сверка при появлении нового
    /// соседа. Но пока соседи на месте, никто не появляется, и сверять историю
    /// оказывается некому. В локальной сети это лечил маячок: он зовёт сверку
    /// каждые пять секунд, и потерянное подбиралось само. Через интернет
    /// маячка нет — и ровно поэтому «на одном вайфае работает, на разных нет».
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for space in self.ctx.space_list() {
                    ...
                    // минуту оказывается не с кем — ровно тогда, когда это
                    ...
                    // недостающие события, совпадают — один короткий раунд.
                    for peer in peers {
                        self.clone().catch_up(space.id, peer);
                    }
                }
