...
impl App {
    ...
    pub async fn music_stop(&self) {
        ...
        // Себе — напрямую: рой не возвращает отправителю его же сообщения,
        // и через него ведущий о себе не узнаёт ничего.
        self.ctx.note_player(music.space, None);
        // Остальным — вслух, а не дать протухнуть: иначе у слушателей ещё шесть
    ...
    fn beat_player(self: Arc<Self>) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            loop {
                ...
                //
                // Рой устроен так, что отправитель своих же сообщений не
                // получает, — и ведущий оставался единственным, кто панели
                // общего плеера не видел вовсе. Собственную музыку он включал
                // вслепую: кнопки появлялись у всех, кроме него.
                self.ctx.note_player(space, Some(state.clone()));
