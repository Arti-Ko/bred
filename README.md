...
- **Отправить файл** — кнопка `+` слева от строки ввода или команда `/файл`.
  ...
  Подпись не обязательна: можно отправить один файл.
- **Удалить своё сообщение** — кликните по нему и нажмите «удалить».
- **Удалить канал** — правый клик по нему в списке слева.
...
- **Развернуть на весь экран** — кнопка «развернуть». Когда идёт демонстрация,
  она занимает основную часть, камеры уходят полосой вниз; кнопка «только экран»
  убирает и их. `Esc` возвращает обратно.
- **Звук вместе с экраном** — включается сам, если система его отдаёт. На macOS
  она его не отдаёт, и приложение об этом честно скажет.
- `Ctrl+D` — выключить микрофон, `Ctrl+E` — выйти из звонка.

### Слушать вместе

> **Раздача плеера пока скрыта.** Кнопку «слушать вместе» из панели звонка
> временно убрали: раздавать свой плеер нельзя, слушать чужой — по-прежнему
> можно. Ниже описано, как это работает, когда кнопку вернут.

Кнопка **«слушать вместе»** в панели звонка включает на комнату звук вашего
музыкального приложения — Яндекс Музыки, браузера, чего угодно. У всех
появляется один общий плеер.

- **Громкость у каждого своя.** Вы поставили себе 50%, сосед 70% — это не
  спорит: регулятор местный и музыку у других не трогает.
- **Кнопки общие.** Пауза, следующий и предыдущий трек нажимаются кем угодно и
  доезжают до самого приложения-источника: музыка встаёт и у вас, и в Яндекс
  Музыке. Никакого общего таймлайна нет — есть пульт, и он один на всех.
- **«Играет» — это не то, что нажали, а то, что слышно.** Остановите музыку
  прямо в приложении — плеер у всех покажет паузу.
- Ведущий один на пространство: две музыки поверх друг друга слушать нельзя.

Что нужно системе на macOS, один раз:

- **Запись экрана** — иначе звук чужого приложения не отдаётся вовсе. Диалог
  появится сам при первом включении.
- **Универсальный доступ** — без него не доедут пауза и переключение трека:
  так устроены системные медиа-клавиши. Разрешение выдаётся в «Конфиденциальность
  и безопасность → Универсальный доступ».

На Windows разрешений не нужно ни одного, но нужна **Windows 11 или Windows 10
сборки 20348 и новее**: на более старых система не умеет отдавать звук
отдельного приложения, и БРЕД честно об этом скажет.

Если в списке источников нужного приложения нет или из него идёт тишина —
возьмите **«весь звук системы»**. Электронные приложения играют музыку из
процесса-помощника, и по имени их поймать удаётся не всегда.
